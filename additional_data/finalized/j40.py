...
Mass,Rad,prob, name = read40("j40/NICER+XMM-relative_J0740_RM.txt")
M_grid = np.linspace(min(Mass), max(Mass),50)
R_grid = np.linspace(min(Rad), max(Rad),50)
n = len(M_grid)
...
plt.colorbar()
plt.savefig("mr_j40.png",dpi=300, bbox_inches='tight')

### write the data 
with open('j0040_likelihood.txt', 'w') as f:
    
    f.write('# Column 1: Equatorial circumferential radius in kilometers \n')
    f.write('# Column 2: Gravitational mass in solar masses \n')
    f.write('# Column 3: weight\n')

    for i in range(n):
        for j in range(n):
            f.write(str(R_grid[j])+ " " + str(M_grid[i])+ " " + str(Z[i][j])+"\n")
