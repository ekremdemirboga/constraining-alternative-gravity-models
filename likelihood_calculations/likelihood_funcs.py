...
def MassRadius(datafile,type): #returns mass,radius,prob2d for each data file
    #"MRprob_J0740_miller.txt","MRprob_J0740_riley.txt","MRprob_J0030_miller.txt","MRprob_J0030_riley.txt"
    firstColumn,secondColumn,thirdColumn,name = MRprobRead("MRprob2"+"/"+datafile)
    print(name)
    if (type==1):
        if name+".txt" in ["MRprob_J0740_miller.txt","MRprob_J0740_riley.txt","MRprob_J0030_miller.txt","MRprob_J0030_riley.txt"]:
            if name+".txt" == "MRprob_J0740_miller.txt":
                n_mass= 202
                n_radius = 201
            elif name+".txt" == "MRprob_J0740_riley.txt":
                n_mass= 201
                n_radius = 202
            elif name+".txt" == "MRprob_J0030_riley.txt":
                n_mass= 202
                n_radius = 202
            elif name+".txt" == "MRprob_J0030_miller.txt":
                n_mass= 201
                n_radius = 201
            Radius = np.asarray(np.sort(list(set(secondColumn))))
            Mass = np.asarray(firstColumn[:n_mass])
...
def Repeated(Mass_array,Mass_value):
    for i in range(len(Mass_array)):
        if (abs(Mass_value - Mass_array[i])<1e-5):
            return True
    return False
...
def interpolated(R_stt,M_stt,A_r,R_gr,M_gr,Model):
    ...
    if Model == 'linear/positive':
        ...
        m_stt,fR_stt = arrange_quad(m_stt,fR_stt)
    else:
...
def dataReader2(directory):
    ...
    with open(directory) as f:
        ...
        for line in f:
            ...
            if(abs( (float(this_line[5])) -mas)<1e-5 or abs( float(this_line[7])-rad)<1e-5):
                continue
    ...
    if GR == 1:
        ...
        for i in m:
            ...
            if probability_data == "MRprob_M28.dat" or probability_data == "MRprob_OmCen.dat" or probability_data == "MRprob_NGC6304.dat" or probability_data == "MRprob_NGC6397.dat" or probability_data == "MRprob_M30.dat" or probability_data =="MRprob_M13.dat":
                ...
                if Mass>2.1:
                    Probability =0
            elif probability_data == "MRprob_J0740_riley.txt":
                if (Mass>2.4 or Mass<1.75) or (Radius<8 or Radius>16):
                    Probability=0
            elif probability_data == "MRprob_J0740_miller.txt":
                if (Mass>2.4 or Mass<1.6) or (Radius<9 or Radius>20):
                    Probability=0
            elif probability_data == "MRprob_J0030_riley.txt":
                if (Mass>2.3 or Mass<1.0) or (Radius<7 or Radius>16):
                    Probability=0
            elif probability_data == "MRprob_J0030_miller.txt":
                if (Mass>2.25 or Mass<1.0) or (Radius<9 or Radius>19):
                    Probability=0
            else:
                if Mass<0.6:
                    Probability =0
...
def plot(directory, directoryGR , Model,GR_on=1, Madm_R = 1):
    ...
    mass_Baryon, mass_ADM, A_r, radius,phi_c, rho_c, ctr, beta_st, mass_st, eos = dataReader2(directory)
    plt.rcParams.update({
...
def main(relaxation_data,GRdata, probability_data,savedir,type,Model):
    ...
    if len(m_gr1)!=0:
        ...
        if probability_data == "MRprob_M28.dat":### smooth m28
            ProbvsMass_gr1 = savgol_filter(ProbvsMass_gr1, round_up_to_odd(len(ProbvsMass_gr1)/3), 3)
            ProbvsMass_gr1 = MRprob_M28_correct(ProbvsMass_gr1,m_gr1)
        ProbvsMass_gr1 = Correct_p(ProbvsMass_gr1)
    ...
    if probability_data == "MRprob_M28.dat":
        ProbvsMass = savgol_filter(ProbvsMass, round_up_to_odd(len(ProbvsMass)/6), 3) ### smooth m28
        ProbvsMass = MRprob_M28_correct(ProbvsMass,m_stt)
    ProbvsMass = Correct_p(ProbvsMass)
    ...
    if len(m_gr2)!=0:
        ...
        if probability_data == "MRprob_M28.dat":### smooth m28
            ProbvsMass_gr2 = savgol_filter(ProbvsMass_gr2, 5, 3)
            ProbvsMass_gr2 = MRprob_M28_correct(ProbvsMass_gr2,m_gr2)
        ProbvsMass_gr2 = Correct_p(ProbvsMass_gr2)
    ...
    if len(m_gr1)!=0:
        ...
        Integral1 =0
    if len(m_gr2)!=0:
        Integral3 = simpson(ProbvsMass_gr2, m_gr2/pp_mass_conv)
    else:
        Integral3 =0
    ...
    ## giving half weight for the same stars.
    if probability_data in ["MRprob_J0740_miller.txt","MRprob_J0740_riley.txt","MRprob_J0030_miller.txt","MRprob_J0030_riley.txt"]:
        print("half weight for riley and miller data!")
...
def MRprob_M28_correct(p,m):
    for i in range(len(p)):
        if (m[i]/pp_mass_conv)<0.5125:
            p[i] = 0
    return p
def Correct_p(p):
    for i in range(len(p)):
