...
def plot2(directory,savedir, directoryGR, GR_on=1, Madm_R = 1,rhoc_Madm=0):
    ...
    if (Madm_R==1):
        ...
        plt.plot(radius*r_scale*A_r, mass_ADM/pp_mass_conv,'.')
    ...
    if (rhoc_Madm==1):
        ...
        plt.xlim([6,15])
        if (GR_on==1):
            plt.plot (rho_cGR*rho_scale,mass_ADMGR/pp_mass_conv,'k--', label = 'GR unstable')
            plt.plot (rho_cGR_stable*rho_scale,mass_ADMGR_stable/pp_mass_conv, 'k', label = 'GR stable')
        plt.xlabel (r"\rho_c ",fontsize=20)
        plt.ylabel(r"$M_{ADM} / M_{sun}$",fontsize=20)
        title = (r"$M_{ADM}$ vs rho_c \\ for $\beta=$"+str(beta_st*beta_conv)+r" $ m_S =$" + str(mass_st))
        plt.title(title, fontsize=20)
        plt.plot(rho_c*rho_scale, mass_ADM/pp_mass_conv,'.')
        # plt.plot ((fR_stt_1*r_scale),m_stt_1/pp_mass_conv, 'r--', label = 'STT')
        # plt.plot ((fR_stt_2*r_scale),m_stt_2/pp_mass_conv, 'r--')
        # plt.scatter(maxR,maxMass)
        plt.legend()
        plt.savefig(savedir+"Madm_rho_"+ datafile + ".png",dpi=300, bbox_inches='tight')
        plt.close()
