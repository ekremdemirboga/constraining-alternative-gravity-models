...
rho_scale = ((Mass_Sun*1000)/(pp_mass_conv*r_scale**3*10**15))/10**15
...
def dataReader(directory):
    ...
    with open(directory) as f:
        ...
        return massIncreased,mass_Baryon, mass_ADM, A_r, radius,phi_c, rho_c, ctr, beta_st, mass_st, eos

def dataReader_backup(directory):
...
def dataReaderDEF(directory):
    ...
    with open(directory) as f:
        ...
        for line in f:
            ...
            mass_Baryon.append(float(this_line[5]))
            mass_ADM.append(float(this_line[6]))
            A_r.append(float(this_line[7]))
            radius.append(float(this_line[8]))
            phi_c.append(float(this_line[9]))
            rho_c.append(float(this_line[10]))
            ctr.append(float(this_line[11]))  
...
def dataReader3(directory):
    ...
    with open(directory) as f:
        ...
        for line in f:
            ...
            ## categorizing according to instabilities
            if ((int(this_line[1]) == 0)): # if stable 
                Flag = False
            if ((int(this_line[1]) == 1) and Flag): # if stable 
                mass_ADM_stable1.append(float(this_line[5]))
                radius_stable1.append(float(this_line[7]))
                A_r_stable1.append(float(this_line[6]))
                rho_stable1.append(float(this_line[9]))

            if ((int(this_line[1]) == 1) and Flag==False): # if stable 
                mass_ADM_stable2.append(float(this_line[5]))
                radius_stable2.append(float(this_line[7]))
                A_r_stable2.append(float(this_line[6]))
                rho_stable2.append(float(this_line[9]))
            ...
            if ((int(this_line[1]) == 0)): # if stable 
                ...
                radius.append(float(this_line[7]))
                A_r.append(float(this_line[6]))
                rho.append(float(this_line[9]))


        mass_ADM = np.asarray(mass_ADM)
        radius = np.asarray(radius)
        mass_ADM_stable1 = np.asarray(mass_ADM_stable1)
        radius_stable1 = np.asarray(radius_stable1)
        mass_ADM_stable2 = np.asarray(mass_ADM_stable2)
        radius_stable2 = np.asarray(radius_stable2)
        rho = np.asarray(rho)
        rho_stable1 = np.asarray(rho_stable1)
        rho_stable2 = np.asarray(rho_stable2)
        return mass_ADM, radius, mass_ADM_stable1,radius_stable1,mass_ADM_stable2,radius_stable2,A_r,A_r_stable1,A_r_stable2,rho,rho_stable1,rho_stable2
...
def interpolated(R_stt,M_stt,A_r,R_gr,M_gr):
    ...
    m_stt1, fR_stt1,m_stt2, fR_stt2  = arrange(m_stt,fR_stt)

    return fR_stt1,m_stt1, fR_stt2,m_stt2

def interpolated2(rho,M_stt):
    #interpolate GR
    print((M_stt))
    rhostt,m_stt= Interpolated_stt(rho*rho_scale,M_stt/pp_mass_conv)
    m_stt1,rhostt1,m_stt2,rhostt2 = arrange2(m_stt,rhostt)

    return rhostt1, m_stt1, rhostt2, m_stt2
...
def arrange(m_stt,fR_stt):
    ...
    for i in range(len(m_stt)-1):  
        if (m_stt[i+1]>m_stt[i]):
            stable_m_stt.append(m_stt[i])
            stable_fR_stt.append(fR_stt[i])
        else:
            unstable_m_stt.append(m_stt[i])
            unstable_fR_stt.append(fR_stt[i])
    return np.asarray(stable_m_stt), np.asarray(stable_fR_stt),np.asarray(unstable_m_stt), np.asarray(unstable_fR_stt)

def arrange2(m_stt,rho):
    stable_m_stt = []
    stable_rho = []
    unstable_m_stt = []
    unstable_rho = []
    for i in range(len(m_stt)-1):  
        if (m_stt[i+1]>m_stt[i]):
            stable_m_stt.append(m_stt[i])
            stable_rho.append(rho[i])
        else:
            unstable_m_stt.append(m_stt[i])
            unstable_rho.append(rho[i])
    return np.asarray(stable_m_stt), np.asarray(stable_rho),np.asarray(unstable_m_stt), np.asarray(unstable_rho)
...
def plot2(directory,savedir=r"./", directoryGR=GR_dir_eos3, GR_on=1, Madm_R = 1):
    ...
    if (Madm_R==1):
        ...
        plt.plot(radius*r_scale, mass_ADM/pp_mass_conv,'.')
